      ...
      {categories.map((cat) => {
        ...
        return (
          <div key={cat} className="mb-14">
            <h2 className="font-heading text-3xl font-bold text-[#1E332E] mb-6 pb-2 border-b border-black/10">
              {cat}
            </h2>
            <div className="grid md:grid-cols-3 gap-8">
              {catProducts.map((item, idx) => (
                <div key={idx} className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 border border-black/5 shadow-md hover:-translate-y-1 transition-all flex flex-col justify-between">
                  <div>
                    <img src={item.image} alt={item.name} className="w-36 h-36 object-cover mx-auto rounded-full mb-6 shadow-lg" />
                    <span className="inline-block text-[10px] font-bold tracking-wider px-3 py-1 rounded-full bg-[#FEF9EC] text-[#B88E28] border border-[#F7E08B] mb-2">
                      {item.badge}
                    </span>
